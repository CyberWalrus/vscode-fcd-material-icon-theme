...
export const getConfigProperties = (): { [config: string]: unknown } => {
  return extensions.getExtension('CyberWalrus.fcd-material-icon-theme')
...
export const getThemeConfig = (section: string) => {
  return getConfig('fcd-material-icon-theme').inspect(section);
};
...
export const setThemeConfig = (
  ...
  return getConfig('fcd-material-icon-theme').update(section, value, global);
};
...
const getExtensionPath = () =>
  extensions.getExtension('CyberWalrus.fcd-material-icon-theme')
    ?.extensionPath ?? '';
